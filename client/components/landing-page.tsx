...
function Button({ children, href, variant = "primary", className = "" }: ButtonProps) {
  ...
  return (
    <button type="button" className={`${baseClasses} ${variantClasses} ${className}`}>
      {children}
    </button>
  );
}
...
function HeroSection() {
  return (
    // Reduced top padding (pt-2 lg:pt-6) and bottom padding to bring everything up
    <section className="relative z-0 mx-auto flex max-w-5xl flex-col items-center px-4 pt-2 pb-12 text-center sm:px-6 lg:px-8 lg:pt-6">
      {/* Background Gradients */}
      <div className="pointer-events-none absolute left-1/2 top-0 h-[28rem] w-[28rem] -translate-x-1/2 rounded-full bg-emerald-200/30 blur-[120px]" />
      <div className="pointer-events-none absolute right-0 top-24 h-80 w-80 rounded-full bg-sky-200/20 blur-[100px]" />
      <div className="pointer-events-none absolute bottom-10 left-0 h-72 w-72 rounded-full bg-amber-100/20 blur-[100px]" />

      <div className="relative z-10 w-full max-w-3xl">
        {/* Top Badge - reduced bottom margin */}
        <div className="mx-auto mb-4 inline-flex items-center gap-2 rounded-full border border-black/5 bg-white/60 px-3 py-1.5 text-sm font-medium text-gray-600 shadow-sm backdrop-blur-md">
          <span className="h-2 w-2 rounded-full bg-emerald-500" />
          AI agents connected to the web
        </div>

        {/* Headings */}
        <h1 className="text-4xl font-semibold tracking-tight text-[#2f2d2b] sm:text-5xl md:text-6xl">
          Connect your AI agents
        </h1>
        {/* Reduced margin top to keep headings tight */}
        <h2 className="mt-1 bg-gradient-to-r from-gray-800 to-emerald-600 bg-clip-text text-5xl font-semibold tracking-tight text-transparent sm:text-6xl md:text-7xl">
          to the web
        </h2>

        {/* Subtitle - reduced top margin */}
        <p className="mx-auto mt-4 max-w-2xl text-lg leading-relaxed text-black/60 sm:text-xl">
      ...
      {/* Search Box Wrapper - significantly reduced top margin (mt-8 instead of mt-12/16) */}
      <div id="search-box" className="relative z-10 mt-8 w-full max-w-4xl">
        <SearchBox />
      </div>
    </section>
  );
...
export default function LandingPage() {
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#f7f4ed] text-[#2e2c2a]">
      ...
      {/* Header - slightly reduced vertical padding from py-6 to py-4 to save vertical space */}
      <header className="relative z-10 mx-auto flex max-w-6xl items-center justify-between px-5 py-4 sm:px-8 lg:px-10">
      ...
      <section id="features" className="relative z-10 mx-auto max-w-7xl px-5 pt-12 pb-20 sm:px-8 lg:px-10 lg:pb-24">
        <SectionHeading
          eyebrow="Features"
